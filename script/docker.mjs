  ...
  ntfPaths.map(async (ntfPath) => {
    ...
    for (const file of ntf.files) {
      if (file.includes('node_modules')) {
        ntfSet.add(
          '.' + file.split('node_modules').slice(1).join('node_modules')
        )
      }
    }
  })
...
await fs.copy(
  new URL('./config-utils.js', import.meta.url).pathname,
  path.resolve(tmpPath, './next/dist/server/config-utils.js')
)
